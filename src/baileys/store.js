...
function bindStore(sock, sessionId, logger) {
  ...
  ev.on('chats.upsert', async (chats) => {
    try {
      await chatRepo.upsertMany(sessionId, chats);
    } catch (err) {
  ...
  ev.on('messages.upsert', async ({ messages, type: upsertType }) => {
    try {
      ...
      for (const m of messages) {
        ...
        chatTouches.push({ jid: m.key.remoteJid, lastMessageId: m.key.id, conversationTimestamp: ts });
      }
  ...
  ev.on('messaging-history.set', async ({ chats, contacts, messages, isLatest, syncType }) => {
    try {
      ...
      if (messages?.length) ev.emit('messages.upsert', { messages, type: 'append' });
    } catch (err) {
